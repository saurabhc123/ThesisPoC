...
import Utilities.{CleanTweet, GenerateCosineSimStatistics, MetricsCalculator, TweetsFileProcessor}
...
class AuxiliaryDataBasedExperiment(args: Array[String]) extends IExperiment {
	...
	def handleProgramArguments(): Unit = {
		...
		if (args.length > firstArgumentIndex + 6)
			AuxiliaryDataBasedExperiment.experimentSetNumber = args(firstArgumentIndex + 6)
	...
	override def performExperiment(train: RDD[Tweet], validation: RDD[Tweet]): Unit = {
		...
		while (f1 < thresholdF1 && numberOfIterations < AuxiliaryDataBasedExperiment.maxExperimentIterations) {
			...
			if (AuxiliaryDataBasedExperiment.classifierType == ClassifierType.Cnn) {
				...
				//Only the other classifiers need these features.
...
object AuxiliaryDataBasedExperiment {
	...
	def setCosineFilteringParameters() = {
		val trainingTweets = TweetsFileProcessor.LoadTweetsFromFileNoCounter(AuxiliaryDataBasedExperiment.trainingDataFile, AuxiliaryDataBasedExperiment.fileDelimiter)
		val (minSimilarityThreshold, cosineSimilarityWindowSize) = GenerateCosineSimStatistics.getCosineSimParametersForTweets(trainingTweets)
		AuxiliaryDataBasedExperiment.minSimilarityThreshold = minSimilarityThreshold
		AuxiliaryDataBasedExperiment.cosineSimilarityWindowSize = cosineSimilarityWindowSize
	}
